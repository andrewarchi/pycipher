...
class Rot13(Cipher):
    ...
    where c is the ciphertext letter, p the plaintext letter. This is equivalent to the Caesar
    cipher with a key of 13. For more details on the rot13 cipher, see
    ...
    def encipher(self, string, keep_punct=False):
        ...
        for c in string:
            if c.isalpha():
                ret += self.i2a(self.a2i(c) + 13)
            else: ret += c
        return ret

    def decipher(self, string, keep_punct=False):
        r"""Decipher string using rot13 cipher. The Deciphering and enciphering operations are
        identical.
